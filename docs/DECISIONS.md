...
## [2026-08-30] Beslutning 39: Burpees på engelsk, og automatisk halevakt mot avkuttede klipp (absolutt + søsken-relativ)
* **Kontekst:** To QA-funn i samme klasse på rad. (a) Produkteier A/B-lyttet burpees og valgte engelsk uttale — den fonetiske omskrivingen «Børpis» tapte, samme utfall som `mountain-climbers` fikk i QA-runde 1. (b) Chatterbox er stokastisk og kutter av og til halen av et take: «burpees» ble til «burpii», den avsluttende s-lyden manglet. Tidligere fanget produkteier den motsatte enden — amputerte ANSATSER («hulekroppshold» → «ulekroppshold»), løst med 100 ms pre-roll i `TRIM_START` (Beslutning 36). Begge funnene kom fra at et menneske hørte dem; det skal vi ikke være avhengige av.
* **Valg:** (1) `burpees` får `ttsText: "Burpees"` med `ttsLang: "en"` i `scripts/voicebank-manuskript.json`. `displayText` og filnavn er urørt — visningsnavn og filsti er stabile kontrakter mot manifestet og buffer-nøklene. Sidecar-ferskheten (Beslutning 36) invaliderer rå-cachen automatisk siden både tekst og språk endres, så klippene re-hentes uten `--force`. Antall øvelser med språkoverstyring er nå 2, pinnet i test. (2) **Halevakt** i produksjonslinja: etter etterbehandling av hvert klipp måles `mean_volume` for hele klippet og for de siste 50 ms (`-sseof -0.05`, `volumedetect`), og differansen *hale minus samlet* sammenlignes med `TAIL_FALLOFF_THRESHOLD_DB = -15`. Et normalt klipp dør ut (differansen er tydelig negativ); et avkuttet klipp har fortsatt full energi i siste vindu og lander nær null. Strengt over terskelen = mistenkt (nøyaktig −15 er IKKE mistenkt), og ikke-endelige måltall (`-inf` fra digital stillhet) gir aldri advarsel. Terskel, falloff-utregning, ffmpeg-argumenter, parsing og rapporteringstekst er rene funksjoner i `scripts/voicebankTasks.ts` (modulen har fortsatt null I/O); selve målingen bor i runneren. Advarselen navngir fila, måltallene og foreslår `--refetch --only <id> --persona <persona>` for et nytt take, og sluttrapporten teller og lister de flaggede klippene. **Kjøringen avbrytes aldri og exit-koden er urørt** — et take kan legitimt være brått. (3) **Søsken-relativ vakt** ved siden av den absolutte, fordi den absolutte alene *ikke* ville fanget funnet den ble bygget for. «Søsken» = samme cue-/øvelses-id generert for de ANDRE personaene fra samme manuskripttekst. Hale-fallet er nokså likt på tvers av personaer for samme ytring, så `assessSiblingTail` sammenligner klippets fall med **medianen** av søsknenes og flagger strengt over `SIBLING_TAIL_DEVIATION_DB = 20` dB (nøyaktig 20 er IKKE mistenkt — samme kant-semantikk som den absolutte). Et klipp som ligger LAVERE enn medianen (lengre utdøing) flagges aldri. Terskelen er lest av begge ender av dataene: burpii-klippet lå ~46 dB over søskenmedianen, mens de 6 klippene den absolutte vakten flagger (og som trolig bare er brå avslutninger) har en innbyrdes spredning på ~11,5 dB — 20 dB ligger i det tomme rommet mellom normal persona-variasjon og et faktisk avkuttet take. **Minst 2 brukbare søsken kreves;** med færre er dommen `{ status: 'not-assessed' }` i en union, ikke `false` — «ikke vurdert» skal ikke kunne forveksles med «frisk» på kallstedet. Siden dommen forutsetter at søsknene er målt, samler runneren målingene underveis og kjører søsken-passet TIL SLUTT, før sluttrapporten. Konsollvarsel og sluttrapport merker hvilken vakt som slo ut (samme klipp kan stå under begge), og ved delvise kjøringer (`--persona`/`--only`) skriver rapporten eksplisitt at søsken-vakten *ikke kunne vurdere* — fraværet av søsken-flagg er da ikke en friskmelding. Også denne vakten er kun en advarsel: exit-koden er urørt.
* **Konsekvens:** Terskelen er kalibrert mot et fullt sveip av lydbanken 2026-08-30 (152 klipp). Sveipet ga **6 flaggede klipp**, ikke null: `hardcore/exercise-hulekroppshold` (−1,7 dB), `hardcore/bro-naa` (−4,1), `hardcore/exercise-goblet-squat` (−9,4), `romsdal/exercise-skulder-dislocates` (−11,4), `romsdal/bro-naa` (−12,9), `boyband/exercise-utfall-forover` (−13,2). Typisk normalklipp faller −25 dB eller mer, spennet går ned mot −72 dB. De seks er **ikke verifisert avkuttet** — de er kandidater for en lytt, ikke bekreftede feil, og illustrerer at terskelen bevisst er satt for å fange den grove klassen framfor å tie. **Hvorfor begge vaktene trengs:** de bommer på hver sin måte, i hver sin retning. Den absolutte tier om det opprinnelige «burpii»-klippet (−24,4 dB mot søskenklipp på −50 til −72 — det ble avslørt av SAMMENLIGNINGEN med søsknene, og ville ikke blitt fanget av −15-terskelen), men er den eneste som virker per klipp, altså også i en delvis kjøring eller på en id der bare én persona er generert. Den søsken-relative fanger nettopp burpii-klassen — klipp som dør ut «litt», men langt mindre enn søsknene sine — og har ingen mening om personaer eller tekster som naturlig slutter brått, siden alle søsknene da ligger like høyt; til gjengjeld er den blind uten et bredt nok sveip. Til sammen dekker de både «halen har full energi» og «halen avviker fra alle søsknene», og begge er advarsler: ingen av dem avbryter kjøringen eller rører exit-koden.
