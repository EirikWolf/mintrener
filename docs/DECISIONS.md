...
## [2026-08-30] Beslutning 39: Burpees på engelsk, og automatisk halevakt mot avkuttede klipp
...
* **Valg:** (1) `burpees` får `ttsText: "Burpees"` med `ttsLang: "en"` i `scripts/voicebank-manuskript.json`. `displayText` og filnavn er urørt — visningsnavn og filsti er stabile kontrakter mot manifestet og buffer-nøklene. Sidecar-ferskheten (Beslutning 36) invaliderer rå-cachen automatisk siden både tekst og språk endres, så klippene re-hentes uten `--force`. Antall øvelser med språkoverstyring er nå 2, pinnet i test. (2) **Halevakt** i produksjonslinja: etter etterbehandling av hvert klipp måles `mean_volume` for hele klippet og for de siste 50 ms (`-sseof -0.05`, `volumedetect`), og differansen *hale minus samlet* sammenlignes med `TAIL_FALLOFF_THRESHOLD_DB = -15`. Et normalt klipp dør ut (differansen er tydelig negativ); et avkuttet klipp har fortsatt full energi i siste vindu og lander nær null. Strengt over terskelen = mistenkt (nøyaktig −15 er IKKE mistenkt), og ikke-endelige måltall (`-inf` fra digital stillhet) gir aldri advarsel. Terskel, falloff-utregning, ffmpeg-argumenter, parsing og rapporteringstekst er rene funksjoner i `scripts/voicebankTasks.ts` (modulen har fortsatt null I/O); selve målingen bor i runneren. Advarselen navngir fila, måltallene og foreslår `--refetch --only <id> --persona <persona>` for et nytt take, og sluttrapporten teller og lister de flaggede klippene. **Kjøringen avbrytes aldri og exit-koden er urørt** — et take kan legitimt være brått.
* **Konsekvens:** Terskelen er kalibrert mot et fullt sveip av lydbanken 2026-08-30 (152 klipp). Sveipet ga **6 flaggede klipp**, ikke null: `hardcore/exercise-hulekroppshold` (−1,7 dB), `hardcore/bro-naa` (−4,1), `hardcore/exercise-goblet-squat` (−9,4), `romsdal/exercise-skulder-dislocates` (−11,4), `romsdal/bro-naa` (−12,9), `boyband/exercise-utfall-forover` (−13,2). Typisk normalklipp faller −25 dB eller mer, spennet går ned mot −72 dB. De seks er ikke verifisert avkuttet — de er kandidater for en lytt, og illustrerer at terskelen bevisst er satt for å fange den grove klassen framfor å tie. **Kjent begrensning:** en absolutt terskel alene skiller ikke alle tilfeller. Det opprinnelige «burpii»-klippet målte −24,4 dB mot søskenklipp på −50 til −72 dB — det ble avslørt av SAMMENLIGNINGEN med søsknene, og ville ikke blitt fanget av −15-terskelen. En søsken-relativ vakt (avvik fra medianen for samme tekst på tvers av personaer) er den naturlige neste iterasjonen om falske positive eller bomskudd blir et problem i praksis.
