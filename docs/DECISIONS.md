...
* **Måletoleranse — hvorfor tallene pinnes med ±1,0 dB:** Tabellen over er målt med **ffmpeg 8.1.1** på eierens Windows-maskin. CI kjører `ubuntu-latest`, som typisk har ffmpeg 6.x — to hovedversjoner eldre. Integrasjonstesten asserterte opprinnelig disse tallene med `toBeCloseTo(fall, 1)`, altså **±0,05 dB**, og det er en tidsinnstilt CI-flake: den slår til ved neste runner-image-oppdatering, på en test hvis egentlige verdi ligger et helt annet sted. Risikoen er undersøkt empirisk. **Dekodingen er ikke problemet** — `-c:a mp3` og `-c:a mp3float` gir identiske tall på det avkuttede klippet (hele −17,7 / hale −42,1), så dekodervalg er deterministisk. **Driftskilden er `-sseof` sin søkepresisjon:** søker en eldre ffmpeg til en annen mp3-rammegrense, forskyves 50 ms-vinduet, og siden én mp3-ramme er ~26 ms — over halve måletvinduet — endrer måltallet seg langt mer enn 0,05 dB. Testen skiller derfor nå eksplisitt mellom to slags assertions. **Klassifiseringen** (`isTailSuspect`: det avkuttede flagges, de åtte friske flagges ikke, inversjons-regresjonsvakten) er det vakten faktisk gjør, hviler på marginer på 11,6–13,9 dB og er **streng** — den tåler enhver rimelig versjonsdrift. **De eksakte dB-tallene** er kalibreringsdokumentasjon og pinnes med en navngitt konstant `MAALING_TOLERANSE_DB = 1.0`: de skal fange en reell drift i måleoppsettet (flyttet måling, endret vindue, byttede ffmpeg-argumenter — det gir titalls dB), ikke rammegrense-støy mellom ffmpeg-versjoner. Én følge: rangeringen «`befal_start_321_short` (−49,4) er dårligere enn dårligste verifisert friske (−49,9)» har bare 0,5 dB margin, altså under toleransen, og pinnes derfor **ikke** som en ordning i testen — bare som en dokumentert måling. Punkt (c) under står ved lag som observasjon, men er ikke noe testen kan innestå for på tvers av ffmpeg-versjoner.
