...
* **Valg:** (1) Hodrommet er ikke en konstant, men summen av `audioBufferEngine.getDuration()` for nøyaktig de klippene den reaktive kjeden vil spille i fasen. Nøkkelutledningen (`deriveAnnounceChain`) har ÉN definisjon og TO kallsteder — avspillingsveiene og hodroms-utregningen — slik at et senere bytte av kjeden automatisk oppdaterer hodrommet. (2) Degraderingsrekkefølge når kjeden ikke rekker fram til fasegrensen: full kjede → dropp bro → dropp cue → (får ikke engang navnet plass) behold dagens fulle kjede. Produkteiers klage er at NAVNET mangler, så navnet kuttes aldri bevisst — bedre å bli preemptet enn å tie. Kun persona-stien; standard-stien (TTS) er uendret. (3) Hodromsgaten gjelder KUN fasestart-veien: `handleDeadlineChanged` (pause/resume, dvale-reanker, catch-up) og `replanCurrentPhase` re-utsteder lookaheaden uten å spille annonseringen på nytt, og en aktiv gate ga der total stillhet inn mot grensen (verken 3-2-1 eller pip, siden gate-grenen bevisst ikke setter `beepFallback`). (4) `start_321_short` er ikke lenger en 8,5 s hale av innspillingene, men en egen TTS-cue per persona; den erstatter det gamle klippet kun med `--only start_321_short --force` (skip-eksisterende-logikken i `voicebankTasks.ts` beskytter ellers den gamle fila).
* **Konsekvens:** 765 grønne tester; fasiten (23 hook-tester) byte-identisk. Fasittesten pinner de reelt målte hardcore-lengdene i en 10 s pause: ingen endAt-nedtellingskjede skeduleres, og kjeden degraderes til øvelsesnavnet alene. Testmockene for `audioBufferEngine.has()`/`getDuration()` deler nå ett buffer-lager — de leser samme Map i produksjon, og «cachet uten varighet» er en verden som ikke finnes. Gjenstår: ny felttest på Android for å bekrefte at navnet nå faktisk høres.
